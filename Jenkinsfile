pipeline {
    ...
    stages {
        ...
        stage('Deploy on K8s for development'){
            ...
            steps {
                sh "helm upgrade --install --wait --namespace ${env.Namespace_dev} --set microservice.name=product --set image.repository=${env.ImageName}_dev --set image.tag=${env.imageTag} product-dev ./charts"
            }
        }
        stage('Deploy on K8s for production'){
            ...
            steps {
                sh "helm upgrade --install --wait --namespace ${env.Namespace} --set microservice.name=product --set image.repository=${env.ImageName} --set image.tag=${env.imageTag} product ./charts"
            }
        }
